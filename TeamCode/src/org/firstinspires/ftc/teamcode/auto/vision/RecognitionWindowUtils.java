...
public class RecognitionWindowUtils {
    ...
    public RobotConstants.RecognitionResults lookThroughWindows(Point pCenterOfObject, Mat pRecognitionObjectOut, String pOutputFilenamePreamble,
        ...
        if (pCenterOfObject.x >= leftWindowData.first.x && pCenterOfObject.x < leftWindowData.first.x + leftWindowData.first.width) {
            RobotLogCommon.d(TAG, "Success: Object found in the left recognition window: location " + leftWindowData.second);
            ...
            if (pCenterOfObject.x >= rightWindowData.first.x && pCenterOfObject.x < rightWindowData.first.x + rightWindowData.first.width) {
                RobotLogCommon.d(TAG, "Success: Object found in the right recognition window: location " + rightWindowData.second);
                foundLocation = rightWindowData.second;
            } else {
                RobotLogCommon.d(TAG, "Object not found in the left or recognition window: assuming location " + nposWindowData.second);
    ...
    public static Mat overlayRecognitionWindows(ImageProvider pImageProvider,
        ...
        //**TODO Stuck because the directory structure of the testbed is not the same as that
        // of IJCenterStageVision or Android.
